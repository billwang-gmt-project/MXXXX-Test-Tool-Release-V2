...
| # | Loop | Pole | Standby | Drive mode | PWM polarity | Hall | SoftSwitch | → mode |
|---|------|------|---------|------------|--------------|------|------------|:------:|
| C1 | Close | 4p | – | Direct-PWM | Non-invert | non-FR | Square | **5** |
| C2 | Close | 4p | – | Direct-PWM | Non-invert | FR | Square | **6** |
| C3 | Close | 6p / 8p | Shutdown | Direct-PWM | Non-invert | FR | Square | **4** |
| O1a | Open | 4p / 8p | Shutdown | Direct-PWM | Invert | – | Square | **0** |
| O2a | Open | 4p / 8p | Shutdown | Direct-PWM | Non-invert | – | Square | **7** |
| O3 | Open | 4p / RD | Shutdown | Direct-PWM | Non-invert | – | 22.5° | **1** |
| O4 | Open | 4p / 8p | – | Direct-PWM | Non-invert | FR | Square | **2** |
| O5 | Open | 4p / 6p | – | Direct-PWM | Non-invert | non-FR | Square | **3** |

**重點差異說明**（硬體工程師確認）：

- **DC mode 已不在判定支援範圍** — 規則表 Drive mode 欄全為 Direct-PWM，DC mode 任何組合一律回 Error（mode = null）。
- **O1/O2 群組**（mode 0 / 7，4p/8p、shutdown）— 差別在 PWM 有無反相（O1a Invert → 0；O2a Non-invert → 7）；另外 mode 7 無 force startup 功能。
...
| 2 | §2.1 Mode 判定表完整性 | 8 列已涵蓋全部合法輸入組合、互斥不漏；O1/O2 / O3/O4/O5 群組差異另見 §2.1 重點差異說明 |
...
function ClassifyMode(in) -> int? :
    // 2026-05-28 update：規則表加入 Drive=DirectPWM 嚴格欄位 + 加嚴 PwmPolarity/SoftSwitch。
    // DC mode 任意組合不在 8 條規則內 → fall through → return null。

    if in.LoopType == Close:
        // C1: Close + 4p + DirectPWM + NonInvert + NonFR + Square
        if in.Pole == 4p and in.DriveMode == DirectPWM and not in.PwmInvert
           and in.HallType == NonFR and in.SoftSwitch == Square:    return 5  // C1
        // C2: Close + 4p + DirectPWM + NonInvert + FR + Square
        if in.Pole == 4p and in.DriveMode == DirectPWM and not in.PwmInvert
           and in.HallType == FR and in.SoftSwitch == Square:       return 6  // C2
        // C3: Close + 6p|8p + Shutdown + DirectPWM + NonInvert + FR + Square
        if (in.Pole == 6p or in.Pole == 8p) and in.Standby == Shutdown
           and in.DriveMode == DirectPWM and not in.PwmInvert
           and in.HallType == FR and in.SoftSwitch == Square:       return 4  // C3
        return null

    // Open-loop
    // O1a: Open + 4p|8p + Shutdown + DirectPWM + Invert + (Hall=–) + Square
    if (in.Pole == 4p or in.Pole == 8p) and in.Standby == Shutdown
       and in.DriveMode == DirectPWM and in.PwmInvert
       and in.SoftSwitch == Square:                                 return 0  // O1a

    // O2a: Open + 4p|8p + Shutdown + DirectPWM + NonInvert + (Hall=–) + Square
    if (in.Pole == 4p or in.Pole == 8p) and in.Standby == Shutdown
       and in.DriveMode == DirectPWM and not in.PwmInvert
       and in.SoftSwitch == Square:                                 return 7  // O2a

    // O3: Open + 4p|RD + Shutdown + DirectPWM + NonInvert + (Hall=–) + Deg22_5
    if (in.Pole == 4p or in.Pole == RD) and in.Standby == Shutdown
       and in.DriveMode == DirectPWM and not in.PwmInvert
       and in.SoftSwitch == Deg22_5:                                return 1  // O3

    // O4: Open + 4p|8p + (Standby=–) + DirectPWM + NonInvert + FR + Square
    if (in.Pole == 4p or in.Pole == 8p) and in.DriveMode == DirectPWM
       and not in.PwmInvert and in.HallType == FR
       and in.SoftSwitch == Square:                                 return 2  // O4

    // O5: Open + 4p|6p + (Standby=–) + DirectPWM + NonInvert + NonFR + Square
    if (in.Pole == 4p or in.Pole == 6p) and in.DriveMode == DirectPWM
       and not in.PwmInvert and in.HallType == NonFR
       and in.SoftSwitch == Square:                                 return 3  // O5
...
| 8 | mode = 7 分類條件 | `(pole==4p\|8p) & shutdown & (PWM_non_invert or DC_mode)` 單條 | ~~拆兩條：**O2a** Direct-PWM + Non-invert、**O2b** DC mode + (non-FR+VSP \|\| FR+SET)~~（2026-05-28 起 O2b 已移除，DC mode 一律 Error；見 `docs/superpowers/specs/2026-05-28-m8160-mode-classification-refactor-design.md`） | – |
