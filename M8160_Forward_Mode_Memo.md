...
function Calculate(inputs) -> ResultRow[]:
    ...
    if mode in {4,5,6}:                              // close-loop
        ...
        ratio = Rpm1RatioTable[inputs.RpmRatioIndex]
        rp1Upper = 187 * ratio                        // RP1 7-bit saturation (MinRpm 上限)
...
>
> - **RP1 (7-bit, MinRpm)**：`(59·ratio, 187·ratio)` — 例如 ratio=2.54 → (149.86, 474.98)
...
AutoPickRpmRatio(MinRpm, MaxRpm):
    ...
    rp1Factor = 187    // RP1 7-bit upper-bound coefficient
...
**為何不再用 `CloseLoopSsTable` 的 `(lo, hi)`：** 該表的 `hi` 欄 = RP2 max（1079·ratio），漏掉 RP1 max（187·ratio < RP2 max）。舊版邏輯讓 (MinRpm, MaxRpm) 在 RP1 已 saturate 時仍被當成「fit」，硬體實際 MinRpm 操作點被 clamp 到 RP1 field=127。雙 bound 對齊既存 [`M8160SpeedCurveCalculator.SelectRpmRatio`](../../../Helpers/M8160SpeedCurveCalculator.cs)（已正確使用 `Rp1Factor` / `Rp2Factor`）。

**雙 bound 預算表（pass 1 接受區間）：** RP1 區間 `MinRpm ∈ (59·ratio, 187·ratio)`；RP2 區間 `MaxRpm ∈ (59·ratio, 1079·ratio)`。

| idx | rpm_ratio | RP1 區間（MinRpm ∈） | RP2 區間（MaxRpm ∈） |
| :-: | :-: | :-: | :-: |
| 0 | 2.54 | (149.86, 474.98) | (149.86, 2740.66) |
| 1 | 3.72 | (219.48, 695.64) | (219.48, 4013.88) |
| 2 | 5.59 | (329.81, 1045.33) | (329.81, 6031.61) |
| 3 | 8.30 | (489.70, 1552.10) | (489.70, 8955.70) |
| 4 | 12.20 | (719.80, 2281.40) | (719.80, 13163.80) |
| 5 | 17.79 | (1049.61, 3326.73) | (1049.61, 19195.41) |
| 6 | 25.93 | (1529.87, 4848.91) | (1529.87, 27978.47) |
| 7 | 37.96 | (2239.64, 7098.52) | (2239.64, 40958.84) |
...
| --- | --- | --- |
| (500, 4000)  | idx=0：500 > 474.98（RP1 上界 fail）→ idx=1：500 ∈ (219.48, 695.64) ✓ 且 4000 ∈ (219.48, 4013.88) ✓ | **idx=1** |
| (1000, 4000) | idx=0：1000 > 474.98 fail → idx=1：1000 > 695.64 fail → idx=2：1000 ∈ (329.81, 1045.33) ✓ 且 4000 ∈ (329.81, 6031.61) ✓ | **idx=2** |
| (600, 2500)（觸發舊 bug） | idx=0：600 > 474.98 fail（舊邏輯誤判通過）→ idx=1：600 ∈ (219.48, 695.64) ✓ 且 2500 ∈ (219.48, 4013.88) ✓ | **idx=1** |
...
| 2 | **rp2** `reg_07[7:0]` | mode∈{4,5,6}：`field = clamp(round(MaxRpm/rpm_ratio/4 − 59), 0, 255)`（**含 −59 offset**，2026-05-08 spec）；其他 mode 不寫此 row | mode∈{4,5,6}：ypFix → **226**；否則 `field = adc2[7:0]`（直接 8-bit、無 −20）。**mode∉{4,5,6} → 0** | close-loop **不對稱**：Forward 含 `−59`、Reverse `rp2 = adc2`（直接相等，無 offset），兩端不再互為逆運算。Forward pin V = `rp2 × 5/256`（Allen direct，**無 +20**），對應 Reverse `yp_pin[7:0] = reg_07[7:0]`。Compare 視窗 rp2 會出現固定 ±59 register field 差異（spec 容許）；ypFix 分支見 #6；open-loop Forward 缺 row → Compare 由 Reverse `0` expected 補齊。**邊界注意**：當 Forward field ≥ 246（pin V ≥ 4.805V，命中 fix-H 門檻）或 field ≤ 9（pin V ≤ 0.176V，命中 fix-L 門檻），Reverse 會改走 #6 的 ypFix 分支 → 226，導致 Compare 在 ±59 之外再疊加 fix 邊界 mismatch（詳見「邊界 mismatch 區間」）。**RPM range 預先檢查**：`AutoPickRpmRatio` / `CalcSs` OutOfRange 已用 RP1/RP2 雙 bound 過濾掉 RP1 saturation 場景（`MinRpm < 187·ratio`），但 RP2 fix-H/fix-L 仍可能觸發（與 RP1 飽和無關，是 ADC 端電壓邊界） |
| 3 | **yp1** `reg_04[6:0]`（close-loop 常數） | mode∈{4,5,6}：固定寫 `12` (≈4.6%，2026-05-08 從 26 改 12) | mode∈{4,5,6}：固定 `26` | **2026-05-08 不對稱**：Forward yp1 = 12（對齊「YP1 固定 4.6%」spec），Reverse hardcode 維持 26；Compare 視窗會顯示 yp1 mismatch（12 vs 26）。如要消除此 mismatch，需另行決定是否同步更新 Reverse hardcode |
...
| 6 | **yp1 / rp2 fix**（fix-H / fix-L 端點） | mode∈{0,1,2,3,7} + `\|Yp1Duty − 4.64\| < 0.005`：寫 `yp1 = 12`，pin V = 5V / 0V | ypV 命中 fix 端（IsFixH：V ≥ 4.8V；IsFixL：V ≤ 0.195V）：mode∈{0,1,2,3,7} → `yp1 = 12`、mode∈{4,5,6} → `rp2 = 226` | open-loop 兩端 hardcode 同 field=12；close-loop **不對稱**：Forward 不偵測 fix 端（永遠走 row 2 的 `round(MaxRpm/rpm_ratio/4 − 59)` 公式），Reverse 看到 Forward 算出的 pin V ≥ 4.8V 或 ≤ 0.195V 會單側 hardcode 226，造成 Compare mismatch |
...
| --- | --- | --- | --- |
| **rp1** `reg_06[6:0]` | `field = round(MinRpm/rpm_ratio − 59)`，pin V = `(field+20) × 5/256` | `field ≥ 226` → V ≥ 4.805V → Reverse `xp1Fix → 1` | `field ≤ 0`（clamp 下限）+ `(0+20) × 5/256 = 0.391V` → 無法觸發（fix-L 門檻 0.195V） |
| **rp2** `reg_07[7:0]` | `field = round(MaxRpm/rpm_ratio/4 − 59)`，pin V = `field × 5/256`（Allen direct，無 +20） | `field ≥ 246` → V ≥ 4.805V → Reverse `ypFix → 226` | `field ≤ 9` → V ≤ 0.176V → Reverse `ypFix → 226` |
...
| OutOfRange | 紅底白字 (`#7A1B1B` / White) | close-loop SS：RP1/RP2 雙 bound 任一不滿足（`MinRpm ∈ (59·ratio, 187·ratio)` 且 `MaxRpm ∈ (59·ratio, 1079·ratio)`，2026-05-08 spec） | ✓ |
...
| 6 | Close-loop SS 範圍檢查 | `min_rpm < RPM < max_rpm` 單一條件 | RP1/RP2 雙 bound：`MinRpm ∈ (59·ratio, 187·ratio)` 且 `MaxRpm ∈ (59·ratio, 1079·ratio)`；越界 → `OutOfRange`（2026-05-08 spec；舊版單看 `tableMaxRpm = 1079·ratio` 漏 RP1 max） | `6c9b9b89` |
...
if mode in {4, 5, 6}:                          // Close-loop：§6.3 對照表
    ...
    rp1Up   = 187 * ratio                       // RP1 7-bit saturation (MinRpm 上限)
...
- **#7（fr_pin_sw 條件式）/ #8（mode=7 拆 O2a/O2b）**：設計原稿邏輯不嚴謹，Allen reverse calculator 已拆分，Forward 對齊。
