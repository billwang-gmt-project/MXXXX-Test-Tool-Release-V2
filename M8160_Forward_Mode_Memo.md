...
| 6 | **yp1 / rp2 fix**（4.64% open circuit） | mode∈{0,1,2,3,7} + `\|Yp1Duty − 4.64\| < 0.005`：寫 `yp1 = 12`，pin V = 5V / 0V | ypV 命中 fix 端（IsFixH/IsFixL）：mode∈{0,1,2,3,7} → `yp1 = 12`、mode∈{4,5,6} → `rp2 = 226` | open-loop 兩端 hardcode 同 field=12；close-loop Forward 不觸發開路語意（mode 4/5/6 永遠寫 26/127），Compare 對 `rp2 = 226` 由 Reverse 單側提供 |
