...
判定 mode ─┬─→ mode ∈ {0,1,2,3,7}  (open-loop)
           ...
           └─→ mode ∈ {4,5,6}      (close-loop)
                   ...
                   │
                   └─ SS  V        = (xp1/yp Fix?) ? CloseLoopSsFixVoltage[idx] : CloseLoopSsTable[idx]
...
function Calculate(inputs) -> ResultRow[]:
    ...
    if mode in {4,5,6}:                              // close-loop
        // 2026-05-29 spec：XP1 或 YP 任一 pin V 落在 ADC rail band（Fix-H/Fix-L）→ 用 Fix 表（旋轉版）
        anyPinFix = IsFixVoltage(xp1PinV) or IsFixVoltage(ypPinV)   // M8160AdcTable.IsFixVoltage
        ssV = anyPinFix ? CloseLoopSsFixVoltage[inputs.RpmRatioIndex]
                        : CloseLoopSsTable[inputs.RpmRatioIndex].SsVoltage
        ratio = Rpm1RatioTable[inputs.RpmRatioIndex]
...
> **2026-05-29 update**：mode 0 / mode 7 從 `non-FR + SET` / `non-FR + VSP` 簡化為單純 `non-FR`（移除 VSP/SET 子條件）。mode 0/1/7 現在統一為 non-FR → V_high、FR → V_low。
...
CloseLoopSsTable (RpmRatioIndex 0..7):    SS(NotFix)  SS(Fix)
  0  ratio=2.54   範圍 149.86..2740.66      0.00 V     4.06 V
  1  ratio=3.72   範圍 219.48..4013.88      0.92 V     5.00 V
  2  ratio=5.59   範圍 329.81..6031.61      1.55 V     0.00 V
  3  ratio=8.3    範圍 489.70..8955.70      2.18 V     0.92 V
  4  ratio=12.2   範圍 719.80..13163.80     2.80 V     1.55 V
  5  ratio=17.79  範圍 1049.61..19195.41    3.43 V     2.18 V
  6  ratio=25.93  範圍 1529.87..27978.47    4.06 V     2.80 V
  7  ratio=37.96  範圍 2239.64..40958.84    5.00 V     3.43 V
```

> 巧合（不是巧合）：Close-loop SS Not-Fix 電壓表 = `ModeVoltageTable[idx]`，因為兩者都是 8-step 0~5V 線性切割。

> **2026-05-29 spec：close-loop SS 雙表**。當 XP1 或 YP 任一 pin V 落在 ADC rail band（Fix-H code 246..255 / Fix-L code 0..9，`M8160AdcTable.IsFixVoltage`）時改用 **Fix 表**（`CloseLoopSsFixVoltage`，即 Not-Fix 表整體旋轉後的版本）；否則用 Not-Fix 表（`CloseLoopSsTable.SsVoltage`）。close-loop 的 Fix 看 RP1/RP2 pin V 是否進 rail band，與 open-loop 的 4.64% open-circuit 屬不同路徑。
...
XP1 pin: SelectXp1HighBranch(mode=7) = (HallType == NonFR) = true → V_high   (2026-05-29 簡化：non-FR → high)
...
if mode in {4, 5, 6}:                          // Close-loop：§6.3 對照表
    // 2026-05-29 spec：XP1 或 YP pin V 落 rail band (Fix-H/Fix-L) → Fix 表（旋轉版）
    anyPinFix = IsFixVoltage(xp1PinV) or IsFixVoltage(ypPinV)
    ssV     = anyPinFix ? CloseLoopSsFixVoltage[RpmRatioIndex]
                        : CloseLoopSsTable[RpmRatioIndex].SsVoltage
...
SelectXp1HighBranch(mode, i):
    if mode == 0:  return i.HallType == NonFR                             // 2026-05-29 簡化：non-FR → high; FR → low
    if mode == 1:  return i.HallType == NonFR
    if mode == 7:  return i.HallType == NonFR                             // 2026-05-29 簡化：non-FR → high; FR → low
...
| 2026-05-29 | XP1 branch mode 0 / 7 簡化為 Non-FR | 使用者新 spec：mode 0 從 `!(FR && VSP)`、mode 7 從 `!(FR && SET)` 一律改為 `HallType == NonFR`，移除 VSP/SET 子條件；mode 0/1/7 統一 non-FR → V_high、FR → V_low。`SelectXp1HighBranch` 兩處 if 簡化。 | (待 commit) |
| 2026-05-29 (續) | close-loop SS 依 XP1/YP Fix 狀態選表 | 使用者新 spec：新增 `CloseLoopSsFixVoltage`（Not-Fix 表旋轉版 `{4.06,5,0,0.92,1.55,2.18,2.8,3.43}`）。XP1 或 YP 任一 pin V 落在 ADC rail band（`M8160AdcTable.IsFixVoltage`）→ Fix 表，否則 Not-Fix 表。`CalcSs` 加 `anyPinFix` 參數，旗標由 `Calculate()` 從 pin `TargetVoltage` 反推。先在 `throwaway/m8160_forward_pin_pseudocode.html` review 後回寫。 | (待 commit) |
