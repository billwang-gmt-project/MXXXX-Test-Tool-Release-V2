...
#### A.9.1 Derive 公式對齊（6 個 derive field）

下表為 Forward 與 Reverse 兩端 **derive 公式** 的對齊點。這些 field 由使用者輸入或 mode 推導而來，兩端公式互為逆運算，Compare 時必須完全相符。
...
#### A.9.2 Compare 容忍清單（5 個 forced field）

下表為 Compare 時被列入 **容忍清單** 的 5 個 forced field（與 §A.1 對應表「Forced」類型一致）。這些 field 由 Forward 工具強制鎖定為常數，覆蓋使用者 UI 選擇；當兩端硬體欄位不對等時，Compare 將其視為容忍而非 mismatch。
