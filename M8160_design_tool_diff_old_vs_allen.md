...
| # | Field | 項目 | Old | Allen |
|---|-------|------|-----|-------|
| 3 | `rpm_ratio` | xp1Fix 分支 | `ADC3[7:5] + 4` | `mod(ADC3[7:5] + 2, 8)` |
...
| 9 | `icmd_sw` | 常數值 | all modes → **0** | all modes → **1** |
| 10 | `rp1` | non-fix 分支 | `ADC1[6:0] - 20` | `xp1V > 2.5`: `~ADC1[6:0] - 20` ; else: `ADC1[6:0] - 20` |
| 11 | `rp2` | non-fix 分支 | `ADC2[7:0]`（8-bit） | `ADC2[6:0]`（**7-bit**） |
...
|---|-------|-------------|----------------|
| 11 | `rp2` | `reg_07[7:0]`（8 bits） | `reg_07[6:0]`（**7 bits**） |
| 12 | `ss_time` | `reg_08[7:5]`（3 bits） | `reg_08[6:5]`（**2 bits**） |
| 16 | `source_duty` | `reg_0d[6]` | **`reg_02[5]`** |
| 17 | `align_sw` | `reg_0e[7]` | **`reg_0a[5]`** |
| 18 | `low_freq_rev` | `reg_0e[5]` | **`reg_0a[6]`** |
| 19 | `fr_pin_sw` | `reg_0f[1]` | **`reg_08[3]`** |
...
`cmd_inv`、`fan_curve`、`rpm_ratio`（non-fix 分支）、`min_sd`、`soft_sw`、`so_set`、`auto_soft_sw`、`duty_deb_sw`
